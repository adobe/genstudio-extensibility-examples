...
import { Claim, Experience } from "@adobe/genstudio-uix-sdk";
import {
...
import { extractClaimsFromFragment } from "./extractClaimsFromFragment";
...
export const validateClaims = ({
  ...
  selectedFragmentKey: Key;
}) => {
  const claims: Claim[] = extractClaimsFromFragment(
    fragments,
    selectedFragmentKey
  );
  ...
  for (const [fieldName, entry] of Object.entries(experienceFields)) {
    if (typeof entry.fieldValue === "string") {
      ...
      // check if field name is body, header, or pre_header to apply checkclaim for includes and return violated when normalize text doesnt contains the normalized claim
      const baseFieldName = removePodPrefix(fieldName);
      if (["body", "header"].includes(baseFieldName)) {
        for (const claim of claims) {
          const checkResult = checkClaim(entry.fieldValue, claim.description);
          if (checkResult.status === VIOLATION_STATUS.N_A) {
            result[fieldName].push({
              status: VIOLATION_STATUS.Violated,
              violation: CLAIM_VIOLATION_PREFIX + claim.description,
            });
          }
        }
      }
