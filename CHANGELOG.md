...
- Read experiences from host and run validation
  - File: `genstudio-mlr-claims-app/src/genstudiopem/web-src/src/components/ValidationPanel.tsx`
  - New (SDK):

```ts
const experiences = await ValidationExtensionService.getExperiences(
  guestConnection
);
```
...
```ts
const experiences =
  await guestConnection.host.api.validationExtension.getExperiences();
```
...
const payload: AdditionalContext<Item> = {
  extensionId,
